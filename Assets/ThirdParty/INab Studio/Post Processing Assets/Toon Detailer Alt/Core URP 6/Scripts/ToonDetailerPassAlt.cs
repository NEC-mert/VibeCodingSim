...
using static INab.ToonDetailerAlt.URP.ToonDetailerAlt;


namespace INab.ToonDetailerAlt.URP
{
    public class ToonDetailerPassAlt : ScriptableRenderPass
    {
        private ToonDetailerAltSettings m_Settings;
        ...
        public ToonDetailerPassAlt(string passName)
        {
            profilingSampler = new ProfilingSampler(passName);
        }

        public void Setup(ref Material material,ref ToonDetailerAltSettings settings)
        {
            m_Material = material;
            m_Settings = settings;
        }
        ...
        private static void ExecuteMainPass(PassData data, RasterGraphContext context)
        {
            ...
            {
                ToonDetailerVolumeComponentAlt myVolume = VolumeManager.instance.stack?.GetComponent<ToonDetailerVolumeComponentAlt>();
                if (myVolume != null)
                {
                    // Adjustments
        ...
        {
            CoreUtils.SetKeyword(m_Material, ToonDetailerAlt.k_Orthographic, orthographic);

            switch (m_Settings._DetailerType)
            {
                case ToonDetailerAltSettings.DetailerType.Both:
                    m_Material.EnableKeyword(ToonDetailerAlt.k_UseContours);
                    m_Material.EnableKeyword(ToonDetailerAlt.k_UseCavity);
                    break;
                case ToonDetailerAltSettings.DetailerType.Contours:
                    m_Material.EnableKeyword(ToonDetailerAlt.k_UseContours);
                    m_Material.DisableKeyword(ToonDetailerAlt.k_UseCavity);
                    break;
                case ToonDetailerAltSettings.DetailerType.Cavity:
                    m_Material.DisableKeyword(ToonDetailerAlt.k_UseContours);
                    m_Material.EnableKeyword(ToonDetailerAlt.k_UseCavity);
                    break;
                default:
            ...
            {
                m_Material.EnableKeyword(ToonDetailerAlt.k_FadeOn);
            }
            else
            {
                m_Material.DisableKeyword(ToonDetailerAlt.k_FadeOn);
                m_Material.DisableKeyword(ToonDetailerAlt.k_FadeContoursOnly);
            }


            if (m_Settings._FadeAffectsOnlyContours && m_Settings._UseFade)
            {
                m_Material.EnableKeyword(ToonDetailerAlt.k_FadeContoursOnly);
                m_Material.DisableKeyword(ToonDetailerAlt.k_FadeOn);
            }
            else
            {
